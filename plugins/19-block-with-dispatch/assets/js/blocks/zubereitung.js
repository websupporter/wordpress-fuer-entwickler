...
const createHigherOrderComponent = compose([withSelect(select => {
  ...
  return {
    onChange: () => {
      var list = wp.data.select('core/editor').getEditedPostAttribute('categories');
...
registerBlockType('rezepte/zubereitung', {
  ...
  edit: () => {
    return wp.element.createElement(
      'h2',
      null,
      wp.element.createElement(VeganComponent, null)
    );
  },
